...
namespace LemonCat.Areas.Admin.Controllers
{
    public class LoginController : Controller
    {
        ...
        public ActionResult Login(LoginModel model)
        {
            ...
            {
                bool result = UserDAO.Instance.Login(model.UserName, Encryptor.MD5Hash(model.Password));
                if (result)
                {
                    ...
                    Session.Add(CommonConstaints.USER_SESSION, usersession);
    ...
    new { controller = "Lock", action = "Index", id = UserDAO.Instance.getIDbyUserName(model.UserName) }));
        ...
        public ActionResult CreateNewAdmin(string Username, string Email, string Password)
        {
            ...
            {
                int id = UserDAO.Instance.CreateFirstAdmin(Username, Password, Email);
                if (id > 0)
                    return View("Index");
            }
            return View();
