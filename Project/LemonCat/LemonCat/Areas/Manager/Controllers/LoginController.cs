...
namespace LemonCat.Areas.Manager.Controllers
{
    public class LoginController : Controller
    {
        ...
        public ActionResult Login(LoginModel model)
        {
            ...
            {
                bool result = UserDAO.Instance.ManagerLogin(model.UserName, Encryptor.MD5Hash(model.Password));
                if (result)
                {
                    ...
                    Session.Add(CommonConstaints.MANAGER_USER_SESSION, usersession);
    ...
    new { controller = "Lock", action = "Index", id = UserDAO.Instance.getIDbyUserName(model.UserName) }));
        ...
        public ActionResult CreateNewAdmin(string Username, string Email, string Password)
        {
            if (!UserDAO.Instance.IsExistUserName(Username) && !UserDAO.Instance.IsExistEmail(Email))
            {
                int id = UserDAO.Instance.CreateFirstManager(Username, Password, Email);
                if (id > 0)
                    return View("Index");
            }
            return View();
